...
export const migrate = () => {
    const messagesDatabase = getMessages();
    for (const messages of Object.values(messagesDatabase)) {
        for (const comment of messages[messages.length - 1].comments) {
            comment.id = v4();
        }
    }
    saveMessages(messagesDatabase);
}