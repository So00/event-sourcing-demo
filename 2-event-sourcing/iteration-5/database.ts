...
class MessageEventsHandler {
    ...
    private onMessageAdded(event: MessageAdded) {
        ...
        addMessageHistory(event.data.id, event.data.content);
    ...
    private onCommentAdded(event: CommentAdded) {
        ...
        addCommentHistory(event.data.id, event.data.content);
        saveMessages(messages);
    }
    ...
    private onLikeAdded(event: LikeAdded) {
        const messages = getMessages();
        const message = messages[event.data.id];
        const like = {username: event.data.username};
        message.likes.push(like);
        addLikeHistory(event.data.id, message.likes.map(like => like.username));
        saveMessages(messages);
    }
...
export const migrate = () => {
    ...
    fs.writeFileSync(messageHistoryDatabasePath, "{}");
    fs.writeFileSync(commentHistoryDatabasePath, "{}");
    fs.writeFileSync(likeHistoryDatabasePath, "{}");
    saveMessages({});
