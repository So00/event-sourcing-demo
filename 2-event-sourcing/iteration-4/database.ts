...
class MessageEventsHandler {
    ...
    private onMessageEdited(event: MessageEdited) {
        const messages = getMessages();
        const messagesOfId = messages[event.data.id];
        const newMessage = {
            ...messagesOfId[getLastId(messagesOfId)],
            content: event.data.content
        };
        messagesOfId.push(newMessage);
        saveMessages(messages);
    }

    private onCommentAdded(event: CommentAdded) {
        const messages = getMessages();
        const messagesOfId = messages[event.data.messageId];
        messagesOfId[getLastId(messagesOfId)].comments.push({
            id: event.data.id,
            author: event.data.author,
            content: event.data.content
        });
    ...
    private onLikeAdded(event: LikeAdded) {
        ...
        const newMessage = {
            ...messagesOfId[getLastId(messagesOfId)],
            likes: [...messagesOfId[getLastId(messagesOfId)].likes]
        };
        newMessage.likes.push({username: event.data.username});
        messagesOfId.push(newMessage);
        saveMessages(messages);
    }
    ...
    private onLikeRemoved(event: LikeRemoved) {
        ...
        newMessage.likes = newMessage
            .likes
            .filter(like => {
                return like.username !== event.data.username
            })
        messagesOfId.push(newMessage);
        saveMessages(messages);
    }
...
export const getLastId = (messages: IMessage[]) => {
...
export const migrate = () => {
    const events = new EventRepository().getEvents();
    saveMessages({});
    const listIdMessage = Object.keys(events);
    const messageEventsHandler = new MessageEventsHandler();
    for (const id of listIdMessage) {
